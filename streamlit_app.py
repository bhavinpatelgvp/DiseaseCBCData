...
# Page configuration
st.set_page_config(
    page_title="Disease Prediction Application",
    page_icon="🏥",
    layout="wide",
...
# Sidebar navigation
st.sidebar.title("🏥 Medical Records CRUD System")
...
# DISEASE PREDICTION FUNCTIONALITY - COMMENTED OUT FOR CRUD-ONLY MODE
...
def predict_disease(patient_data):
    """CRUD-only mode - No disease prediction."""
    return "Not Available", "Disease prediction disabled in CRUD-only mode", "N/A", 0.0

# Main application pages
if page == "Dashboard":
    st.title("🏥 Medical Records CRUD Dashboard")
    ...
    if not patients_df.empty:
        ...
        recent_patients = patients_df.head(5)[['id', 'name', 'age', 'gender', 'blood_group', 'created_at']]
        st.dataframe(recent_patients, use_container_width=True)

        # Blood group distribution instead of disease distribution
        if 'blood_group' in patients_df.columns and not patients_df['blood_group'].isna().all():
            st.subheader("Blood Group Distribution")
            blood_group_counts = patients_df['blood_group'].value_counts()
            fig = px.pie(values=blood_group_counts.values, names=blood_group_counts.index,
                        title="Distribution of Blood Groups")
            st.plotly_chart(fig, use_container_width=True)

        # DISEASE DISTRIBUTION SECTION COMMENTED OUT
        ...
        if submitted:
            if not patient_data['name'] or not patient_data['age']:
                ...
                try:
                    ...
                    # Display patient info instead of prediction
                    st.subheader("Patient Information Saved")
                    col1, col2 = st.columns(2)

                    with col1:
                        st.info(f"**Name:** {patient_data['name']}")
                        st.info(f"**Age:** {patient_data['age']}")
                        st.info(f"**Gender:** {patient_data['gender']}")

                    with col2:
                        st.info(f"**Blood Group:** {patient_data['blood_group']}")
                        st.info(f"**Patient ID:** {patient_id}")
                        st.info("**Status:** Record Created")
    ...
    if not patients_df.empty:
        ...
        if show_all_columns:
            ...
            # Show essential columns (removed prediction for CRUD-only mode)
            essential_cols = ['id', 'name', 'age', 'gender', 'blood_group', 'hb', 'wbc', 'plt', 'created_at']
                ...
                with st.form("update_patient_form"):
                    ...
                    if submitted:
                        if not updated_data['name'] or not updated_data['age']:
                            ...
                            updated_data['prediction'] = "Not Available"
                            ...
                            try:
                                ...
                                if success:
                                    ...
                                    # Display updated patient info instead of prediction
                                    st.subheader("Updated Patient Information")
                                    col1, col2 = st.columns(2)

                                    with col1:
                                        st.info(f"**Name:** {updated_data['name']}")
                                        st.info(f"**Age:** {updated_data['age']}")
                                        st.info(f"**Gender:** {updated_data['gender']}")

                                    with col2:
                                        st.info(f"**Blood Group:** {updated_data['blood_group']}")
                                        st.info(f"**Patient ID:** {patient_id}")
                                        st.info("**Status:** Record Updated")
...
st.sidebar.info(
    "This is a Medical Records CRUD application built with Streamlit. "
    "It allows you to manage patient records with full Create, Read, Update, Delete operations."
)

st.sidebar.markdown("### CRUD Features")
st.sidebar.markdown(
    "- ➕ **Create:** Add new patients\n"
    "- 👥 **Read:** View all patients\n"
    "- ✏️ **Update:** Modify patient records\n"
    "- 🗑️ **Delete:** Remove patients\n"
    "- 🔍 **Search:** Find specific patients\n"
...
st.sidebar.warning(
    "🚫 Disease prediction functionality is currently disabled. "
    "This version focuses on pure CRUD operations for medical records management."
)
